...
namespace SistemaUniversidad.DISEÑO.Administrador
{
    public partial class ModificarAlumno : Form
    {
        ...
        #region Buscar Indice
        private void dgvAlumnos_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            DataGridViewRow filaSeleccionada = dgvAlumnos.SelectedRows[0];
            carnet = filaSeleccionada.Cells[3].Value.ToString();
            edit = dgvAlumnos.Rows.IndexOf(filaSeleccionada);
        }
        private int ElementoEditar(int carrera)
        {
            int indice = 0,editar = 0;
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            if (edit != -1)
            {
                ...
                {
                    Modificar.setResive(ElementoEditar(1), 1);
                }
                else if (cmbCarreras.Text == "Ingeniería Agronómica")
                {
                    Modificar.setResive(ElementoEditar(2), 2);
                }
                else if (cmbCarreras.Text == "Ingeniería Eléctrica")
                {
                    Modificar.setResive(ElementoEditar(3), 3);
                }
